<template>
  <div>
    ...
    <div class="g-mainer pt-30 pb-30 bgproduct" v-if="!investResult.success">
      <div class="wp">
        <div class="mp-detail-total transferdetail">
          <div class="bd">
            <div class="title">
              <ULabel :type="prdTypes[loan.prdType]" size="small" class="ulabel" style="transform:translateY(-2px);"/>
              {{loan.loanTitle}}
              <em>&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;{{loan.repayModeStr}}</em>

              <ULabel class="ml-10" :movegap="1" type="red" size="small" :content="'折价'+parseFloat(view.discountScale*100).toFixed(2)+'%'" v-if="view.discountScale>0"/>
              <ULabel class="ml-10" :movegap="1" type="orange" size="small" content="免垫付利息" notes="受让方无需垫付本期未结利息" v-if="view.isFree==='1'" />
            </div>
            <ul class="details pt-30">
              <li>
                <h2 class="text-warning">{{loan.platRate}}<em>%<span
                  v-if="loan.awardRate&&(loan.awardRate>0)">+{{loan.awardRate}}%</span></em></h2>
                <p>原项目协议约定年化利率
                  <Whattip :notes="whatnotes"/>
                </p>
              </li>
              <li>
                <h2><span class="text-warning">{{view.remainDays}}</span><em>天</em></h2>
                <p>剩余期限</p>
              </li>
              <li>
                <h2 class="text-warning">{{view.transferPrice |formatCurrency}}</h2>
                <p v-if="view.isFree==='1'">转让金额（元）
                  <span class="del" style="text-decoration: line-through;"
                        v-if="view.fairPrice && view.fairPrice>view.transferPrice">{{view.fairPrice|formatCurrency}}</span>
                  <Whattip width="auto" :nowrap="true"
                           :notes="'原价'+view.fairPrice+'元，节省'+parseFloat(view.fairPrice-view.transferPrice).toFixed(2)+'元'"/>
                </p>
                <p v-else>转让金额（元）
                  <Whattip width="auto" :nowrap="true" :notes="'转让金额=债权本金'+formatCurrency(view.transferPrincipal)+'元'+(view.discountScale?('x(1-折价'+view.discountScale*100+'%)'):'')+
                                '+垫付利息'+formatCurrency(view.fairPrice-view.transferPrincipal)+'元'"/>
                </p>
              </li>
            </ul>
            <ul class="details" style="margin:15px 0 0;color:#999;">
              <li style="padding-left: 20px;text-align: left">债权编号:{{view.debtCode}}</li>
              <li>转让方:{{view.fuzzyUserLoginName}}</li>
              <li></li>
            </ul>
          </div>
          <div class="sd success" v-if="view.applyStatus=='3'">
            <div class="statuspic"><img src="@/assets/images/product/transfered.png"></div>
            <h2 class="subscription-title">已转让</h2>
            <p class="text-muted subscription-time">成交时间 {{view.updateTime|dateformat('YYYY-MM-DD HH:mm:ss')}}</p>
          </div>
          <div class="sd" v-else>
            <div class="item">剩余可投（元）</div>
            <div class="amt">{{view.transferPrice |formatCurrency}}</div>
            <div class="item" v-if="pageInfo.loginInfo.isLogin">可用余额 {{user.availableAmt |formatCurrency}} 元
              <router-link to="/spa/member/account/recharge" class="text-primary r"
                           v-if="pageInfo.loginInfo.isLogin">充值
              </router-link>
            </div>
            <div class="item" v-else>可用余额 <a class="text-primary" href="javascript:;"
                                             @click="ifShowLogin=true">登录</a> 可见
            </div>
            <div class="mt-5">
              <InputNumber class="l" :inputkey="'key1'" :step="100" :height="50" :width="305" :showBtn="false"
                           :hasunit="true" :iconSize="30" :fontSize="20" :readonly="true"
                           :max="2000" :defaultValue="view.transferPrice" :type="'currency'"
                           :maxlength="8"></InputNumber>
            </div>
            <div class="btndiv">
              <a class="btn btn-warning btn-block" href="javascript:;" @click="confirmInvest"
                 v-if="pageInfo.loginInfo.isLogin">我要出借</a>
              <a class="btn btn-warning btn-block" href="javascript:;" @click="ifShowLogin=true" v-else>出借请登录</a>
            </div>
          </div>
        </div>
        <div class="mp-detail-details">
          <a-tabs defaultActiveKey="1" size="large" class="ant-tabs-yqd">
            <a-tab-pane tab="原项目详情" key="1">
              ...
              <section class="maincontent" v-else>
                <div v-if="loan.prdType=='3'">
                  ...
                  <div class="item-content">
                    <p
                      class="textIndent2">该项目主要针对群体为具有良好信用记录的个人与企业。在该群体向第三方机构提供不动产作为抵押且获机构对应的增信（包括但不限于担保函、保函或借款保证保险）支持的基础上所提供的信息撮合服务。</p>
                ...
                <div class="item-content">
                  <table class="table detailtable mt-30">
                    <tr>
                      ...
                      <td>
                        <span
                          v-if="user.personFarm && user.personFarm.degree">{{educations[user.personFarm.degree]}}</span>
                ...
                <div v-if="user.picMap &&user.picMap.length>0">
                  ...
                  <div class="enclosure-list">
                    <img :src="plist.fileLink" class="img-fluid" v-for="(plist,index) in user.picMap"
                         :key="'picMap'+index">
                  </div>
                </div>
              </section>
            </a-tab-pane>
            <a-tab-pane tab="原项目风险评估" key="2">
              <div class="maincontent nologin" v-if="!pageInfo.loginInfo.isLogin">
                <img src="@/assets/images/product/login_before.png">
                <p>详细信息请<a href="javascript:;" @click="ifShowLogin=true" class="text-primary">登录</a>后查看</p>
              </div>
              <div class="maincontent" v-else>
                <table class="table detailtable">
                  ...
                  <tr>
                    <td>风险控制措施</td>
                    <td>
                      <div class="defaultfont" v-if="user.riskControl && user.riskControl.riskMeasure"
                           v-html="user.riskControl.riskMeasure"></div>
                    </td>
                  </tr>
                  <tr>
                    <td>风险提示</td>
                    <td>
                      <div class="defaultfont" v-if="user.riskControl && user.riskControl.reminder"
                           v-html="user.riskControl.reminder"></div>
                    </td>
                  </tr>
                  <tr>
                    <td>风险等级</td>
                    <td>
                      <div class="defaultfont" v-if="user.riskGradeStr" v-html="user.riskGradeStr"></div>
                    </td>
                  </tr>
                </table>
              </div>
            </a-tab-pane>
            <a-tab-pane tab="原项目出借记录" key="3">
              <div class="maincontent trustrecord">
                <table class="table recordtable text-c">
                  ...
                  <tbody v-else>
                  <tr>
                    <td colspan="3"><img style="margin:30px 0" class="record" src="@/assets/images/user/record.png">
                    </td>
                  </tr>
                  </tbody>
                </table>
                <div class="page mt-40" v-if="invetsRecord.hasPage">
                  <a-pagination :pageNo="invetsRecord.pageNo" :pageSize="invetsRecord.pageSize"
                                :total="invetsRecord.total" @change="initInvetsRecord"/>
                </div>
              </div>
            </a-tab-pane>
            <a-tab-pane tab="原项目还款计划" key="4" v-if="['6','7','8','9'].includes(loan.loanStatus)">
              <div class="maincontent trustrecord">
      ...
      <div class="subscription-success">
        ...
        <div class="bd">
          <ul class="process process3">
            <li>
              <h4>成功受让此债权{{investResult.investAmount|formatCurrency}} 元</h4>
            ...
            <li>
              <h4>项目到期</h4>
              <span>{{investResult.prdPeriodStr}}， 预期收益{{formatCurrency(view.expectIncome + loan.totalRedAmt + loan.ticketAmt)}}元</span>
            </li>
          </ul>
          <div class="text-c mt-50">
            <router-link class="btn btn-default size-X radius mr-20"
                         to="/spa/member/invest/holdlist?tabType=holdList">查看资产
            </router-link>
            <router-link class="btn btn-warning size-X radius" to="/spa/product/index">继续出借</router-link>
          </div>
        </div>
      </div>
    </div>
    ...
    <a-modal title="出借确认" :visible="ifConfirmInvest" :maskClosable="false" :bodyStyle="layerStyle"
      ...
      <button type="button" class="closemodal" @click="ifConfirmInvest=!ifConfirmInvest"><i
        class="icon-font icon-close"></i></button>
      <div class="m-layer-subscription">
        <div class="bd">
          <table class="table datatable">
            <tbody>
            <tr>
              <td class="text-muted" width="120">项目名称：</td>
              <td>{{loan.loanTitle}}</td>
            </tr>
            <tr>
              <td class="text-muted">债权金额：</td>
              <td><b>{{view.fairPrice|formatCurrency}}</b>元</td>
            </tr>
            <tr>
              <td class="text-muted">转让金额：</td>
              <td><b>{{view.transferPrice|formatCurrency}}</b>元</td>
            </tr>
            <tr>
              <td class="text-muted">选择优惠券
                <Whattip width="275px" notes="平台自动匹配的红包/奖励券不保证符合您的最优选择，以您实际确认使用为准"></Whattip>
                ：
              </td>
              <td>
            ...
            <tr>
              ...
              <td><span class="f-16">{{formatCurrency(view.expectIncome + loan.totalRedAmt + loan.ticketAmt)}}</span> 元
                <Whattip width="auto" :nowrap="true" arrow="right"
                         :notes="'项目收益 '+formatCurrency(view.expectIncome)+(loan.totalRedAmt?(' + 红包收益 '+formatCurrency(loan.totalRedAmt)):'')+(loan.ticketAmt?(' + 奖励券收益 '+formatCurrency(loan.ticketAmt)):'')"/>
              </td>
            </tr>
            </tbody>
          </table>
          <div class="agreementdiv mt-10">
            ...
            <a target="_blank" class="text-primary" :href="'/contract/debt?assetsId='+view.debtId">《债权转让协议》</a>
            <a target="_blank" class="text-primary" href="/contract/debt-risk">《风险及禁止性行为提示书》</a>
          </div>
          <div class="text-center mt-20">
            <button :class="['btn','btn-warning','btn-subscription',disabledSubmit?'disabled':'']" @click="submitInvest"
                    :disabled="disabledSubmit">
              <a-spin size="small" class="mr-5" v-if="disabledSubmit"/>
              确认出借
            </button>
          </div>
    ...
    <a-modal :title="false" :visible="ifShowRisk" :maskClosable="false" :bodyStyle="layerStyle"
      ...
      <RiskTip :user="user" :loan="loan" :pageInfo="pageInfo" :isAssetstRisk="isAssetstRisk"
               @closemodal="ifShowRisk=false"></RiskTip>
    </a-modal>
    <a-modal :title="false" :visible="ifShowDeposit" :maskClosable="false" :bodyStyle="layerStyle"
      ...
      <button type="button" class="closemodal" @click="ifShowDeposit=false"><i class="icon-font icon-close"></i>
      </button>
      <Deposit></Deposit>
    </a-modal>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      formatCurrency(val, floatType = 2) {
        return formatCurrency(val, floatType)
      },
      initData() {
        this.$post('/transfer/detail/' + this.loanId).then(res => {
          if (res.success) {
            this.currentTime = toJson(res).currentTime
            let result = toJson(res).data
            this.loan = result.assetsDetail
            console.log(result.assetsDetail)
            this.view = result.debtView
            this.remainPeriodDays = result.remainPeriodDays
            this.remainPeriod = result.remainPeriod
            this.user = result
      ...
      initVoucher() {
        this.$post('/transfer/trade/coupon?transferApplyId=' + this.loanId).then(res => {
          if (res.success) {
            let result = toJson(res).data
            this.redList = result.vouchers
            this.ticketList = result.ticket
            this.loan.investAmt = this.view.transferPrincipal
            this.loan.transferPrincipal = this.view.transferPrincipal
            this.loan.isTransfer = true
            this.loan.remainDays = this.view.remainDays
            this.loan.remainPeriodDays = this.remainPeriodDays
            this.loan.remainPeriod = this.remainPeriod
            let loanMatch = loanMatchVoucher(this.loan, this.redList, this.ticketList)
            this.loan = {...loanMatch.loan}
            console.log(JSON.stringify(this.redList, loanMatch))
          }
        })
      },
      initInvetsRecord(pageNo) {
        this.$post('/product/invest/record/' + this.loan.loanId, {
          pageNo: pageNo,
          pageSize: this.invetsRecord.pageSize
        }).then(res => {
      ...
      checkInvestAmt() {
        if (this.loan.investAmt > this.user.availableAmt) {
          this.$layer.msg('可用余额不足')
          return false
        }
        return true
      },
      confirmInvest() {
        this.loan.investAmt = this.view.transferPrice
      ...
      submitInvest() { // 出借确认
        if (this.agreement) {
          ...
          let params = {}
          params.couponIds = this.loan.selectedTicket ? this.loan.selectedTicket : this.loan.selectedRedList.join(',')
          params.transferApplyId = this.view.transferApplyId
          this.$post('/transfer/trade/confirm', params).then(res => {
            let result = toJson(res)
