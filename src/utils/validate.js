...
Validator.extend("isPwd", {
  getMessage: field => "请设置6-20位字母+数字的密码",
  validate: value => {
    let checkPwd = /^(?!^(\d+|[a-zA-Z]+|[~!@#$%^&*?]+)$)^[\w~!@#$%^&*?]{6,20}$/;
    let empty = /\s+/g;
    return checkPwd.test(value) && !empty.test(value)
  }
});
Validator.extend("isRealName", {
  getMessage: field => "请输入正确的真实姓名",
  validate: value => {
    let checkVal = /^[\u4E00-\u9FA5]{2,8}(·[\u4E00-\u9FA5]{1,8})*$/;
...
//金额验证
Validator.extend("minNum", {
  getMessage: field => "最小金额为" + field,
  validate: (value, [arg]) => {
    let num = Number(value)
    let param = Number(arg)
    if (param > num) {
      return false
    }
    return true
...
//金额验证
Validator.extend("maxNum", {
  getMessage: field => "最大金额为" + field,
  validate: (value, [arg]) => {
    let num = Number(value)
    let param = Number(arg)
    if (num > param) {
